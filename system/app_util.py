...
def processPOST(
    ...
    if request.method == 'POST':
        ...
        if audio_file:
            ...
            chat_transcript, display, auto_prompt, subject = processExchange(user, True, audio_file_path, subject, user_info=user_info, prior_conversations=prior_conversations)
            return chat_transcript, display, auto_prompt, subject or ""

        # check if text input is provided
        text_input = req.form.get('text')
        if text_input:
            chat_transcript, display, auto_prompt, subject = processExchange(user, False, text_input, subject, user_info=user_info, prior_conversations=prior_conversations)
            return chat_transcript, display, auto_prompt, subject or ""
    #default return
    return [], None, None, ""
...
def processExchange(
    ...
    if subject:
        ...
        chat_transcript, display = main(user, isAudio, input, existing_messages, user_info, prior_conversations=prior_conversations)
    else:
        chat_transcript, display = main(user, isAudio, input, user_info=user_info, prior_conversations=prior_conversations)
